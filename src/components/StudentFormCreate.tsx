...
export class StudentFormCreate extends Component <Props, State> {
    ...
    private readonly radios: { value: string, label: string, name: string }[] = [
    ...
    private handleOptionChange = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    private handleSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        const studentSearch: Student | undefined = this.state.students.find(student => student.sid === this.state.values.sid)

        if (studentSearch != undefined) {  // sid exist
